"""Daily ESPN ADP refresh for the shipped 2026 Draft Board. MARKET DATA ONLY.

Mirrors refresh_board_adp.py. The 180-player universe and Model Proj stay frozen.
This script pulls ESPN ADP, joins it onto that exact 180 via the committed espn_id
map, recomputes within-180 ESPN position ranks, and writes one regenerable overlay.

It never writes Sleeper overlays, phase4_band_2026.csv, talent artifacts, or the
season dataset. Unmatched rows stay blank: ESPN prices are never filled from
Sleeper ADP.
...
breach aborts before writing and leaves the previous ESPN overlay on disk.

In-season pause: same SEASON_START / BOARD_REFRESH_FORCE contract as Sleeper.

Run:   python fantasy/seasonal_projections/refresh_board_espn_adp.py [--force]
"""
...
from fetch_espn_adp import ID_MAP_CSV, fetch_espn_adp, load_espn_id_map
...
OVERLAY = HERE / "board_espn_adp_live_2026.csv"
LOGS_DIR = HERE / "adp_logs"
LEDGER = LOGS_DIR / "espn_refresh_ledger.jsonl"

OVERLAY_CORE_COLS = [
    "player_id", "player", "position", "espn_id", "espn_adp", "espn_pos_rank",
    "refreshed_at",
]
...
def _prior_snapshot(today_name: str) -> pd.DataFrame | None:
    if not LOGS_DIR.exists():
        return None
    snaps = sorted(
        p for p in LOGS_DIR.glob("board_espn_adp_*.csv") if p.name != today_name
    )
    if not snaps:
        return None
...
def build_espn_overlay_full(
    universe: pd.DataFrame,
    fresh: pd.DataFrame,
    id_map: pd.DataFrame,
    source_date: str,
):
    """Refresh ESPN prices for the fixed 180. Blank where ESPN did not match.

    Returns (overlay, coverage). adp_source is "fresh" or "unmatched". Unmatched
    rows keep a blank espn_adp; they are never filled from Sleeper.
    """
    u = universe.copy()
    ...
    ids["espn_id"] = ids["espn_id"].astype("string")
    m = u.merge(ids[["player_id", "espn_id"]], on="player_id", how="left")

    f = fresh.copy()
    f["espn_id"] = f["espn_id"].astype("string")
    f["nn"] = f["player"].map(nmz)
    by_id = f.drop_duplicates("espn_id")[["espn_id", "espn_adp"]].rename(
        columns={"espn_adp": "adp_by_id"}
    )
    m = m.merge(by_id, on="espn_id", how="left")

    by_name = f.drop_duplicates(["nn", "position"])[["nn", "position", "espn_adp"]].rename(
        columns={"espn_adp": "adp_by_name"}
    )
    m = m.merge(by_name, on=["nn", "position"], how="left")
    m["espn_adp"] = m["adp_by_id"].where(m["adp_by_id"].notna(), m["adp_by_name"])
    m["adp_matched"] = m["espn_adp"].notna()
    m["adp_source"] = m["adp_matched"].map({True: "fresh", False: "unmatched"})

    m = m.sort_values(["espn_adp", "player_id"], kind="stable", na_position="last")
    m["espn_pos_rank"] = (
        m.groupby("position")["espn_adp"].rank(method="min", ascending=True).astype("Int64")
    )
...
def main() -> int:
    ...
    if fresh is None or fresh.empty or "espn_adp" not in fresh.columns \
    ...
    try:
        id_map = load_espn_id_map(ID_MAP_CSV)
    except Exception as exc:
        reason = f"aborted: espn id map failed ({type(exc).__name__}: {exc})"
        print(reason)
        ledger({
            "run_ts": run_ts, "source_date": source_date, "status": reason,
            "pull_players": int(len(fresh)), "matched": None,
    ...
    overlay, coverage = build_espn_overlay_full(universe, fresh, id_map, source_date)
    ...
    today_name = f"board_espn_adp_{source_date}.csv"
    prior = _prior_snapshot(today_name)
    mean_abs = None
    movers = []
    if prior is not None and "espn_pos_rank" in prior.columns:
        joined = overlay.merge(
            prior[["player_id", "espn_pos_rank"]],
            on="player_id", how="inner", suffixes=("", "_prev"),
        )
        joined["delta"] = joined["espn_pos_rank"] - joined["espn_pos_rank_prev"]
        if len(joined):
    ...
    print(
        f"espn refresh OK: {matched}/{len(universe)} matched to ESPN ADP "
