...
Status: checked against the current board builder, artifacts, and site page on 2026-08-21.
...
from the frozen artifacts in `../talent/`; (b) the hit probability, 0–100; (c) a rookie-year **season-total
...
an unavailable market projection, not a failed load. The college talent score is read from frozen
artifacts under `../talent/`. Since 2026-07-27 **every position has a dedicated college build**:
...
- **The talent score is read-only at the board boundary.** Its artifacts live in this repository
  under `../talent/`, but `build_rookie_board.py` only joins them. It does not rebuild them or fill
  their gaps.
