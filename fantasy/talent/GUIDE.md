...
## 3. Public artifacts and local build sources

| File | What it is |
|---|---|
| `nfl_{qb,rb,wr,te}_score_2026.csv` | The four frozen NFL score artifacts read by the Draft Board. |
| `college_{qb,rb,wr,te}_score_2026.csv` | The four frozen college score artifacts read by the Rookie Board and Draft Board explorer. |
...
| `build_nfl_*_score.py`, `build_college_*_score.py` | Eight local-only, self-contained build scripts governed by SPEC R34–R41. |
| `config.py`, `facets.py`, `model.py`, `composite.py` | Local-only R29 support modules retained for the legacy build entry points. |
| `SPEC.md`, `*.provenance.json`, `research/`, `tests/` | Local-only design, provenance, research, and build-test records. Licensed inputs are not public. |
| `../../site_pages/page_draft_board.py` | Draft Board renderer for NFL scores and the outside-market projection explorer. |
| `../../site_pages/page_rookie_board.py` | Rookie Board renderer for college scores. |
| `../../tests/test_app_talent_columns.py` | Public regression coverage for the board wiring and score-column contract. |
...
- **Frozen artifacts.** The shipped CSVs are hash-pinned; a legitimate rebuild
  ...
  silently.
- **Weights are owner config.** Every current weight vector is set on football
  logic and recorded in its dated per-position build and spec; no automated search chooses it. Two
...
The one-line version of this document: these are two narrow descriptive measurements,
fenced away from the value columns and documented where they fail. The current board
does not render uncertainty intervals, so sample shrinkage and blank low-volume cells
are the visible uncertainty signals.
