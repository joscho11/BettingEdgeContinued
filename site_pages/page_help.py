...
**Draft & Roster Insights** opens first and adds three chart-first views. Draft Room shows positional timing, runs, backup QB/TE usage and manager tendencies. On All Time, the insight window is Last season, Last 3 seasons (default), or All available seasons. My Team calculates lineup scoring and season facts after requiring four rostered weeks per player-season. Best Values plots drafted players by round, ranks cheap waiver claims ($0-$4) separately so they do not pile on a scatter, and shows every completed bid of $5 or more against production, including players later dropped. Zero-point $5+ bids are a separate ranked bar so they do not stack at zero on the scatter. Cheap waiver charts only include players with four rostered weeks in that season. Trades are graded as got-versus-gave starting-lineup points from the week after the deal. The other manager is the row label. If a manager has many trades, the chart keeps the eight most lopsided results. Free-agent adds stay off the FAAB charts.
...
The **Rivalries** tab has three focused views: **Build a Week**, **Explore a Matchup**, and **League Matrix**. Only the selected view is shown. The builder creates a globally optimized slate for current managers in Classic Rivalries, Maximum Drama, or Fresh Blood mode, with optional history limits, matchup locks, and alternative slates. The explorer compares one series by record, average scoring edge, playoff meetings, current streak, and game margins. The matrix shows each row manager's record against every opponent; green favors the row manager and rose favors the column opponent. Build-a-Week cards use green for 70+ fit, yellow for 50-69, red below 50, and blue for locked matchups.
