...
The **Rivalries** tab begins with a Rivalry Week Builder for the league's current managers. Choose Classic Rivalries, Maximum Drama, or Fresh Blood; optionally limit the history window, lock matchups, and generate another globally optimized slate. Every suggestion explains the evidence behind it. The Matchup Explorer underneath compares any two managers by series, average scoring edge, playoff meetings, current streak, and every game margin. The league-wide heatmap shows each row manager's record against every opponent; green favors the row manager and red favors the column opponent. The complete W–L matrix remains available underneath the charts.
