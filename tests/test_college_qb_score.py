...
def test_draft_board_shows_college_talent_for_rookie_qbs():
    ...
    rookie_art = pd.read_csv(ARTIFACT)
    rookie_art = rookie_art[
        rookie_art["is_2026_rookie"].astype(bool)
        & rookie_art["nfl_player_id"].notna()
    ]
    eligible_ids = set(qb["player_id"]) & set(rookie_art["nfl_player_id"])
    assert eligible_ids, "the frozen board should intersect the shipped rookie-QB artifact"
    assert set(scored["player_id"]) == eligible_ids, (
        "every rookie QB present in both the frozen 24-QB publication and the college "
        "artifact must carry the artifact score—and no non-intersecting QB should gain one"
    )
    assert "Fernando Mendoza" in set(scored["player"])
    ...
    art = rookie_art.set_index("nfl_player_id")["score"]
