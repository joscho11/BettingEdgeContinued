...
**2026 live spread is `cowork_OS/spread_v3_prod` leftover 75/25**, not `betting/models/`. HIGH 192/336 = 57.14%, one-sided 95% Wilson 0.5266. All-bets off the claim. In-repo spread / seasonal / weekly training is under `archive/legacy-inrepo-2026-08-18/`.
...
- A Streamlit **multipage site** for visualization (deployed at joschoanalytics.streamlit.app). 10 pages behind `st.navigation`, not tabs
- GitHub Actions for weekly automated predictions (Tue/Thu/Sun)
- A pre-season fantasy **2026 Draft Board** page (`draft_board_2026.py`) — **rebuilt 2026-07-22** into a **season-projection comparison table** over the independent model's **180**-player 2026 universe (24 QB, 60 RB, 72 WR, 24 TE): the market's draft price + positional rank beside Sleeper's and a from-scratch model's season projections, with the rank gap for each, plus two descriptive talent columns (**NFL Talent Score**, **College Talent Score**) fed by the eight per-position builds in `fantasy/talent/` (SPEC R34–R41, shipped 2026-07-27). Talent scores: `fantasy/talent/`. It **retired the licensed Phase-4 band** as the tab's spine (the frozen band + `talent_index_2026.csv` stay on disk, read-only, for the closed campaign + the ADP refresh). Band research campaign: `fantasy/seasonal_projections/` (`GUIDE.md` / `ARTIFACTS.md` / `PREREGISTRATION.md` + the `joschoanalytics-seasonal-h5-campaign` skill). Live seasonal product: `cowork_OS/projections_v2/`. In-repo seasonal training: `fantasy/projections/ARCHIVED.md`.
...
**Run the 2026 spread stamp (from cowork_OS/spread_v3_prod, not this repo):**
```bash
...
### Core Files
**2026 spread training/inference is archived.** Website 2026 spread is `cowork_OS/spread_v3_prod`. In-repo spread notebooks live under `archive/legacy-inrepo-2026-08-18/betting/`. Frozen pkls in `betting/models/` are the 2025 demo plus Help SHAP hashes. Do not overwrite them. Totals notebooks in `betting/` remain live.

- **`archive/legacy-inrepo-2026-08-18/betting/predict_betting.ipynb`** (was `betting/predict_betting.ipynb`) — **Archived 2026-08-18.** Historical weekly spread papermill. Do not restore as website prod. Cell map below describes the archived notebook.
...
- **`betting/features.py`** — **Single source of truth** for the 85-feature engineering pipeline (Groups 1–10), shared by archived spread notebooks plus live `predict_totals.ipynb` and `totals_model.ipynb`. Plain importable Python (extracted verbatim from the former `features.ipynb` on 2026-06-15 — see Completed Work). Public surface: `build_features`, `build_numeric_features`, the per-group `_build_*` helpers, `FEATURE_COLS_85`, `PROD_FEATURES_35`, `TEAM_MAP`, `norm_name`, `canonicalize_ngs_team`. **Loading pattern** (consumers): add `betting/` to `sys.path`, `import features as _features`, then `globals().update({k:v for k,v in vars(_features).items() if not k.startswith("__")})` — this mirrors the old `exec(globals())` namespace population (including the `_build_*` helpers). Tests live in **`betting/test_features.py`** (hermetic synthetic-data tests, run in CI — see below); the `PROD_FEATURES_35`/`FEATURE_COLS_85` order-hash check is `test_constants_and_order_hashes`.
- **`archive/legacy-inrepo-2026-08-18/betting/features.ipynb`** — Archived thin documentation notebook. Edit `features.py` instead.
- **`app.py`** — **Thin multipage entrypoint, NOT a tab monolith.** The 8-tab app was migrated to page modules in site-revamp Batch 3. It builds **10 `st.Page` objects** and runs `st.navigation(..., position="top")` in three groups — **Fantasy:** 📋 Draft Board (`default=True`), 🧬 Rookie Board, Weekly Fantasy · **Betting:** Weekly Predictions, Track Record, Season Totals (Beta) · **More:** Film Room, League History, Help & Guide. DFS Optimizer (`page_dfs.py`) stays in-repo and off the nav until the optimizer is live. Each `site_pages/page_*.py` module is loaded through `_lazy_render` only when selected; do not restore eager page imports, which initialize every page's dependency graph on each cold process. Shared chrome/data live in `dashboard_chrome.py` / `dashboard_data.py` / `site_pages/page_common.py`, and cross-links go through `nav_registry.PAGES`. `tests/test_site_nav.py::test_every_page_renders_offline_clean` gives every page its own AppTest render, supplemented by the page-specific suites (`tests/test_board_page.py`, `tests/test_betting_pages.py`, `tests/test_fantasy_league_pages.py`, `tests/test_help_page.py`, `tests/test_app_draft_board.py`, `tests/test_app_talent_columns.py`, `tests/test_page_rookie_board.py`). The pure, Streamlit-free helpers (`metric_card`, `get_confidence`, `_md_to_html`, `load_tracker`, `load_totals_tracker`) live in **`dashboard_utils.py`** (unit-tested by `tests/test_dashboard_utils.py`, in CI). Game cards use the sportsbook-style display negation (see Key Constraints) and show a **dashed amber EXPERIMENTAL UNDER badge** when the totals model has a HIGH pick (amber because live 2025 is only at break-even). The Track Record totals section carries a "tracking only — do not bet" banner. **`APP_OFFLINE=1`** disables every network path (GA, nflreadpy, Sleeper) for hermetic AppTest runs. The Draft Board page (`page_draft_board.render` → `draft_board_2026.render()`) was **rebuilt 2026-07-22** into a season-projection comparison table (retired the frozen Phase-4 band) — READ-ONLY over the projection results + `season_dataset` ADP + talent artifacts, descriptive-only, no BUY/FADE/tier/verdict language. (The old inline Draft Value Finder tab and its `value_board_*.csv` were retired 2026-07-12; `build_value_board.py`/`build_draft_board.py`/`board_view.py` stay on disk but no page renders them.)
- **`site_pages/page_futures.py`** — Season Totals (Beta). READ-ONLY over `futures/published/` line_in files from `season_totals_v2_prod` (`python src/publish_site.py`). 32-team projection; certified picks are HIGH only. Language fence: `futures/language_fence.py`. Old M4-c pipeline: `archive/legacy-futures-m4c-2026-08-18/`. Tests: `tests/test_page_futures.py`.
- **`model_explanations.py` / `model_explanations_snapshot.json` / `site_pages/help_models.py`** — Help **How the models work** rundowns for every live prediction product (2026 spread, Season Totals, Model Proj, weekly fantasy, 2025 demo spread/totals, Rookie Board). Eval bars are frozen in `model_explanations.py` and locked to `live_2026.py` plus `futures/published/evidence.json`. Feature cards still come from the checked snapshot; the live app does not import `joblib`/NumPy or unpickle artifacts. Snapshot still hashes all 22 source pkls so a silent swap fails CI; Help only **draws** `DISPLAYED_HELP_IDS`. After replacing a covered pkl, run `python scripts/build_model_explanations_snapshot.py`.
- **`betting/models/`** — Frozen 2025 demo + Help SHAP hashes. **Do not overwrite.** 2026 website spread pkls live in `spread_v3_prod/artifacts/`. See `betting/models/README.md`.
  - `ensemble_prod_model.pkl` — Frozen 2025 demo ensemble (0.75 XGBoost + 0.25 Ridge). Help hashes this file. Not 2026 website prod.
...
- **`archive/legacy-inrepo-2026-08-18/betting/model_comparison.ipynb`** — Archived spread comparison notebook (70 cells). Do not restore as the 2026 retrain path. See dedicated section below (historical).

### Feature Groups (`betting/features.py`)
...
### Automation
`.github/workflows/weekly_predictions.yml` still runs on three cron schedules (Tue 9am ET, Thu 9pm ET, Sun 9am ET). **Spread papermill is retired:** the spread job only echoes that 2026 stamps come from `spread_v3_prod`. Totals still run `predict_totals.ipynb`. **The Tuesday `sports_betting_agent.ipynb` step was COMMENTED OUT 2026-08-03** because it regenerated a fabricated-market artifact, which would have made quarantining it futile. The commit step now stages **only** `predictions_tracker.csv` and `totals_tracker.csv` — `agent_analysis_*.json` is deliberately not staged. Job timeout is 60 min. Each notebook uploads its own failed-notebook artifact on error.
...
- **MLP is comparison-only** — present in the archived `model_comparison.ipynb` Section 17 for benchmarking. Not in `betting/models/` and not used by the 2026 website spread.
...
ILP-based DraftKings NFL Classic lineup optimizer. Uses our weekly fantasy `projected_pts` as the value signal and solves salary-capped roster selection as a binary integer program. Requires `pulp` from `requirements-research.txt`; the live DFS page is static and does not import the solver.
...
> the closed campaign ONLY - neither is an input to the daily ADP refresh (verified 2026-07-27);
> the description below documents the RETIRED band product, for history.
...
`talent_index_2026.csv` (the retired R29-era 2025-efficiency context artifact — frozen on disk,
rendered by nothing, superseded by the per-position builds in `fantasy/talent/`; never blended
...
Gap and on the descriptive magnitude columns (on the retired band that was Top-12 chance and the
NFL Efficiency %ile; on today's rebuilt board it is the Gap and rank columns) plus a bold,
...
`talent_index_2026.csv` stay on disk for the closed campaign ONLY - the ADP refresh reads neither; the page reads the band
...
the overlay; the committed overlay now carries all 245 rows. Tests: `tests/test_board_page.py`,
...
- **Honest numbers:** walk-forward 2021–2025 (n=802) pooled Spearman **+0.689**; **does NOT beat Sleeper**
  ...
  **backtested, not live-validated** (first live test = end of 2026). Board display retires `proj_ppg` and
  ...
  AppTest: `tests/test_page_rookie_board.py` (not yet in CI).

## Talent Scores (`fantasy/talent/`)

Two descriptive columns on the Draft Board — **NFL Talent Score** and **College Talent Score** — plus the
College Talent column on the Rookie Board. **Descriptive context only: neither feeds any other column, and
neither is a projection.** Formulas of record and the full ratification ledger are in `fantasy/talent/SPEC.md`.

**Eight dedicated per-position builds shipped 2026-07-27** (one session, SPEC R34–R41). Each writes its own
`{nfl,college}_{qb,rb,wr,te}_score_2026.csv` plus a `.provenance.json` recording spec, gamma, weights,
method-of-moments k vector, effective weights, anchor pool and the artifact's own md5:

| Build | SPEC | Pool / floor | γ |
|---|---|---|---|
| `build_nfl_qb_score.py` | R34 | ≥300 dropbacks in 2023–25 | 0.55 |
| `build_college_qb_score.py` | R35 | FBS, ≥200 dropbacks | 0.40 |
| `build_college_rb_score.py` | R36 | FBS, ≥75 attempts | 0.60 |
| `build_nfl_rb_score.py` | R37 | ≥100 carries in window | 0.55 |
| `build_college_wr_score.py` | R38 | FBS, ≥150 routes | 0.50 |
| `build_nfl_wr_score.py` | R39 | ≥250 routes in window | 0.55 |
| `build_college_te_score.py` | R40 | FBS, ≥100 routes | 0.60 |
| `build_nfl_te_score.py` | R41 | ≥150 routes in window | 0.55 |

**Which artifact feeds which column.** NFL Talent Score reads the four NFL builds (R34/R37/R39/R41), disjoint
by position. College Talent Score reads the four college builds (R35/R36/R38/R40). **`talent_score_2026.csv`
(the older R29 build) now feeds NO rendered column** — the per-position migration superseded it at every
position. It stays on disk, unregenerated, at pinned md5 `d7c1a57547be4ab8060c053de02aaead` (asserted by
`tests/test_nfl_qb_score.py`) and is read only as a base layer, so a *missing* per-position file degrades to the old
value instead of blanking the column. `rookie_score_2026.csv` (`57b5b6c51637f11a2067bb7249ac7805`) survives
only as the College Talent fallback where a college build has no coverage.

**Key constraints:**
- **Every build must be imported without running.** All eight expose `build()` behind an
  `if __name__ == "__main__":` guard. Six of them originally did the work at module level, so
  `import build_nfl_rb_score` in a test **rewrote the shipped artifact on every pytest run** — invisible
  because the rebuild was byte-identical against a warm nflreadpy cache. `tests/test_nfl_qb_score.py::test_importing_a_build_never_runs_it` now pins this.
- **Board policy differs by position, deliberately.** College RB/WR/TE **REPLACE** the box-score value
  wherever they cover a player (Joseph's direction 2026-07-27); college QB **FILLS BLANKS ONLY** (there were
  no QB rows in the box-score build to overwrite).
...
- Each provenance file must name its **own** spec number. Both TE builds originally carried their WR
  sibling's (college TE said R38, NFL TE said R39); `test_every_provenance_records_its_own_spec` pins it.
- Block weights written into provenance are **computed from `W`, never transcribed** — the NFL TE file once
  hardcoded WR's 0.400/0.175 against TE's real 0.430/0.145.

**Not in CI (gap):** `tests/test_nfl_qb_score.py` and `tests/test_college_qb_score.py` are not in any `test.yml` job
list; they depend on the talent artifacts, which are untracked until committed. Add them once those land.
They are **not the only gap** — eight test files in total are absent from CI (see the Automation section).

**Open items (untouched, need their own session):** winsorization at ±3 in Stage 1 of the college builds
(Rohan Jones tops college TE at 99.0 on a yprr z of +6.56 from one 106-route season; Kittle's NFL TE 99.0 was
checked and is genuine — clipped, not inflated); the college QB clip floor of 40 against the other three at
50; and the measured-but-unapplied SPEC alternatives (RB "P1", WR "P3", TE "P3", `k_blend` units).
