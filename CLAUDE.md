...
- **`model_explanations.py` / `model_explanations_snapshot.json`** — Help-page feature-influence cards are served from a small checked snapshot; the live app does not import `joblib`/NumPy or unpickle/hash 22 production artifacts. `tests/test_model_explanations.py` verifies all recorded hashes against the current artifacts. After intentionally replacing a covered model, run `python scripts/build_model_explanations_snapshot.py`; changed Tree SHAP models require recomputing their pinned summaries first.
